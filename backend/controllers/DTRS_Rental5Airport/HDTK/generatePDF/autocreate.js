...
const {
  ...
  image6PDF_w,
  image6PDF_h,
} = require("../../../../utils/constant_DTRS_Rental5Airport_HDTK");
...
async function autocreate(import_pdf) {
  ...
  const checkfilesXlsx = globSync(
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/db_rental5airport_hdtk.xlsx`
    )
  );
  ...
  if (statusFilePDF == "FoundFilePDF" && statusFileXlsx == "FoundFileXlsx") {
    ...
    for (let countCheck = 0; countCheck < xlData.length; countCheck++) {
      const checkfiles = globSync(
        path.join(
          process.cwd(),
          `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images/${import_pdf.airportPDF
          }-DTRS-PM${import_pdf.timePDF}-${import_pdf.yearPDF}-HDTK-${String(
    ...
    if (statusFoundImage == "FoundImage") {
      ...
      for (let i = 0; i < xlData.length;) {
        ...
        writeFileSync(
          path.join(
            process.cwd(),
            `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/automations/${setData.airportPDF}-DTRS-PM${setData.timePDF}-${setData.yearPDF}-HDTK-${setData.number1PDF}-${setData.number2PDF}-${setData.number3PDF}-${setData.number4PDF}-${setData.number5PDF}-${setData.number6PDF}.pdf`
          ),
          await document.save()
        );
      }
      ...
      async function setRow1(page_1, fontTHSarabunPSKBold, document, setData) {
        ...
        const image_row1_col1 = await document.embedJpg(
          fs.readFileSync(
            path.join(
              process.cwd(),
              `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images/${setData.airportPDF}-DTRS-PM${setData.timePDF}-${setData.yearPDF}-HDTK-${setData.number1PDF}.jpg`
            )
          )
        );
        ...
        const image_row1_col2 = await document.embedJpg(
          fs.readFileSync(
            path.join(
              process.cwd(),
              `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images/${setData.airportPDF}-DTRS-PM${setData.timePDF}-${setData.yearPDF}-HDTK-${setData.number2PDF}.jpg`
            )
          )
        );
      ...
      async function setRow2(page_1, fontTHSarabunPSKBold, document, setData) {
        ...
        const image_row2_col1 = await document.embedJpg(
          fs.readFileSync(
            path.join(
              process.cwd(),
              `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images/${setData.airportPDF}-DTRS-PM${setData.timePDF}-${setData.yearPDF}-HDTK-${setData.number3PDF}.jpg`
            )
          )
        );
        ...
        const image_row2_col2 = await document.embedJpg(
          fs.readFileSync(
            path.join(
              process.cwd(),
              `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images/${setData.airportPDF}-DTRS-PM${setData.timePDF}-${setData.yearPDF}-HDTK-${setData.number4PDF}.jpg`
            )
          )
        );
      ...
      async function setRow3(page_1, fontTHSarabunPSKBold, document, setData) {
        ...
        const image_row3_col1 = await document.embedJpg(
          fs.readFileSync(
            path.join(
              process.cwd(),
              `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images/${setData.airportPDF}-DTRS-PM${setData.timePDF}-${setData.yearPDF}-HDTK-${setData.number5PDF}.jpg`
            )
          )
        );
        ...
        const image_row3_col2 = await document.embedJpg(
          fs.readFileSync(
            path.join(
              process.cwd(),
              `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images/${setData.airportPDF}-DTRS-PM${setData.timePDF}-${setData.yearPDF}-HDTK-${setData.number6PDF}.jpg`
            )
          )
        );
