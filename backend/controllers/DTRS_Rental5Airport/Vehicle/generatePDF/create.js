...
async function create(import_pdf) {
  ...
  if (statusFilePDF == "FoundFilePDF") {
    ...
    writeFileSync(
      path.join(
        process.cwd(),
        `/model/DTRS_Rental5Airport/Vehicle/generatePDF/uploaded/data/${import_pdf.airportPDF}-DTRS-PM${import_pdf.timePDF}-${import_pdf.yearPDF}-Vehicle-${import_pdf.number1PDF}-${import_pdf.number3PDF}-${import_pdf.number5PDF}.pdf`
      ),
      await document.save()
    );
