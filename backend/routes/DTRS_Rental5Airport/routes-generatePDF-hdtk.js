...
const {
  create,
} = require("../../controllers/DTRS_Rental5Airport/HDTK/generatePDF/create");

const {
  autocreate,
} = require("../../controllers/DTRS_Rental5Airport/HDTK/generatePDF/autocreate");
const { count } = require("console");
//

// Upload Image
uploadImageHDTK = async (files, doc) => {
  if (files.image1 != null) {
    ...
    doc.image1 = `${doc.airport}-DTRS-PM${doc.time}-${doc.year}-HDTK-${doc.number1}.${fileExtention1}`;
    let newpath1 =
      path.resolve(
        process.cwd() +
  ...
  if (files.image2 != null) {
    ...
    doc.image2 = `${doc.airport}-DTRS-PM${doc.time}-${doc.year}-HDTK-${doc.number2}.${fileExtention2}`;
    let newpath2 =
      path.resolve(
        process.cwd() +
        "/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images"
  ...
  if (files.image3 != null) {
    ...
    doc.image3 = `${doc.airport}-DTRS-PM${doc.time}-${doc.year}-HDTK-${doc.number3}.${fileExtention3}`;
    let newpath3 =
      path.resolve(
        process.cwd() +
        "/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images"
  ...
  if (files.image4 != null) {
    ...
    doc.image4 = `${doc.airport}-DTRS-PM${doc.time}-${doc.year}-HDTK-${doc.number4}.${fileExtention4}`;
    let newpath4 =
      path.resolve(
        process.cwd() +
        "/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images"
  ...
  if (files.image5 != null) {
    ...
    doc.image5 = `${doc.airport}-DTRS-PM${doc.time}-${doc.year}-HDTK-${doc.number5}.${fileExtention5}`;
    let newpath5 =
      path.resolve(
        process.cwd() +
        "/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images"
  ...
  if (files.image6 != null) {
    ...
    doc.image6 = `${doc.airport}-DTRS-PM${doc.time}-${doc.year}-HDTK-${doc.number6}.${fileExtention6}`;
    let newpath6 =
      path.resolve(
        process.cwd() +
        "/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images"
...
uploadExcelHDTK = async (files, doc) => {
  ...
  if (files.excel[0].originalFilename != null) {
    ...
    if (fileExtention == "xlsx" || fileExtention == "xls") {
      let newpathExcel =
        path.resolve(
          process.cwd() + "/model/DTRS_Rental5Airport/HDTK/generatePDF"
        ) +
        "/" +
        "db_rental5airport_hdtk.xlsx";
...
// Upload PDF HDTK
uploadPDFHDTK = async (files, doc) => {
  if (files.pdfhdtk != null) {
    let fileExtention1 = files.pdfhdtk[0].originalFilename.split(".")[1];
    doc.pdfhdtk = `DTRS_Rental5airport_Template_PDF_HDTK.${fileExtention1}`;
    let newpath =
      path.resolve(
        process.cwd() +
    ...
    await fs.moveSync(files.pdfhdtk[0].filepath, newpath);
  }
};
  ...
  try {
    ...
    form.parse(req, async (error, fields, files) => {
      let import_pdf = {
        ...
        sumImagePDF: await uploadImageHDTK(files, fields),
      };
...
router.post("/autocreate", async (req, res) => {
  try {
    ...
    form.parse(req, async (error, fields, files) => {
      ...
      await uploadImageHDTKGroup(files, fields);
      await uploadExcelHDTK(files, fields);
...
router.post("/addfilePDF-HDTK", async (req, res) => {
  try {
    const form = new formidable.IncomingForm();
    form.parse(req, async (error, fields, files) => {
      let filePDF_HDTK = {
        pdfhdtk: String(fields.pdfhdtk),
        pdfhdtkUpload: await uploadPDFHDTK(files, fields),
      };
...
router.get("/checkfile/download/automation", async (req, res) => {
  ...
  let checkfiles = globSync(
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/automations/*.pdf`
    )
  );
  if (checkfiles == "") {
    res.json({
...
router.get("/download/automation/:filename", async (req, res) => {
  ...
  await compressing.zip.compressDir(
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/automations`
    ),
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/compressing/download_reportPDF_automation_DTRS_Rental5Airport_HDTK.zip`
    )
  );
  ...
  res.download(
    filePath,
    "download_reportPDF_automation_DTRS_Rental5Airport_HDTK.zip",
    (err) => {
      if (err) {
        res.json({
          result: constants.kResultNOk,
...
router.get("/checkfile/download", async (req, res) => {
  ...
  let checkfiles = globSync(
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/data/*.pdf`
    )
  );
  if (checkfiles == "") {
    res.json({
...
router.get("/download/:filename", async (req, res) => {
  // http://localhost:8085/api/generatePDF/dtrs_rental5airport/hdtk/download/download_reportPDF_DTRS_Rental5Airport_HDTK.zip

  await compressing.zip.compressDir(
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/data`
    ),
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/compressing/download_reportPDF_DTRS_Rental5Airport_HDTK.zip`
    )
  );
  ...
  res.download(
    filePath,
    "download_reportPDF_DTRS_Rental5Airport_HDTK.zip",
    (err) => {
      if (err) {
        res.json({
          result: constants.kResultNOk,
...
router.get("/checkfile/download/images", async (req, res) => {
  ...
  let checkfiles = globSync(
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/images/*.jpg`
    )
  );
  if (checkfiles == "") {
    res.json({
...
router.get("/download/images/:filename", async (req, res) => {
  // http://localhost:8085/api/generatePDF/dtrs_rental5airport/hdtk/download/images/download_reportPDF_Images_DTRS_Rental5Airport_HDTK.zip
  ...
  await compressing.zip.compressDir(
    ...
    path.join(
      process.cwd(),
      `/model/DTRS_Rental5Airport/HDTK/generatePDF/uploaded/compressing/download_reportPDF_Images_DTRS_Rental5Airport_HDTK.zip`
    )
  );
  ...
  res.download(
    filePath,
    "download_reportPDF_Images_DTRS_Rental5Airport_HDTK.zip",
    (err) => {
      if (err) {
        res.json({
          result: constants.kResultNOk,
